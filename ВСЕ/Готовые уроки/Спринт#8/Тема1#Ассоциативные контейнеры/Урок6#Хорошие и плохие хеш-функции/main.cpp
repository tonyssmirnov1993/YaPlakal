...
public:
    ...
    VehiclePlate(char l0, char l1, int digits, char l2, int region) : letters_{l0, l1, l2}
                                                                    , digits_(digits)
                                                                    , region_(region) {}
...
struct PlateHasherAll {
    size_t operator()(const VehiclePlate& car) const {
        auto letters = car.GetLetters();
        return size_t(car.GetRegion() * 1000000
                      + ((letters[2] - 'A')
                      + (letters[1] - 'A')
                      + (letters[0] - 'A')) * 1000
                      + car.GetDigits());
...
public:
    VehiclePlate Generate() {
        return VehiclePlate(GenerateChar(), 
                            GenerateChar(), 
                            GenerateNumber(), 
                            GenerateChar(), 
                            GenerateRegion());
