...
//==================== Step 3: set parameters for compression =========================
    ...
    cinfo.image_width = image.GetWidth();  //image width (in pixels)
    cinfo.image_height = image.GetHeight();//image height (in pixels)
...
Image LoadJPEG(const Path& file) {
    ...
    // Тут не избежать функции открытия файла из языка C, поэтому приходится использовать конвертацию пути к string.
    // Под Visual Studio это может быть опасно, и нужно применить нестандартную функцию _wfopen
#ifdef _MSC_VER
